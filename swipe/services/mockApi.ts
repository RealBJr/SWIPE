import { seedCourses, seedProfessors, seedStudents, seedTAs } from '@/data/seed';
import {
...
function buildQueue(segment: ExploreSegment, profile: UserProfile | null) {
  if (segment === 'students') {
    ...
    const ids = seedStudents.map((s) => s.id).filter((id) => !seen.has(id) && !matched.has(id));
...
export async function submitReview(
  input: Omit<Review, 'id' | 'authorName' | 'createdAt'> & { authorName?: string }
) {
